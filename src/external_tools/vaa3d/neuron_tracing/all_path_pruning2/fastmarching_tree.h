 ...
 *          phi          the distance for each pixels
 * *******************************************************************/
template<class T> bool fastmarching_tree(MyMarker root, T * inimg1d, vector<MyMarker*> &outtree,
                           long sz0, long sz1, long sz2, int cnn_type = 3,
                           double bkg_thresh = 20, bool is_break_accept = false)
{
	int8_t ALIVE = -1, TRIAL = 0, FAR = 1;
	...
	while(!heap.empty())
	{
		...
		state[min_ind] = ALIVE;
        i = min_ind % sz0;   // mzhu: variable declaration hides declaration in outer scope
        long j = (min_ind/sz0) % sz1;
        long k = (min_ind/sz01) % sz2;
			...
			for(int jj = -1; jj <= 1; jj++)
			{
				...
				for(int ii = -1; ii <= 1; ii++)
				{
					...
					if(state[index] != ALIVE)
					{
						...
						long prev_ind = min_ind; // mzhu: variable declaration hides declaration in outer scope

						if(state[index] == FAR)
						{
							phi[index] = (float)new_dist;
							HeapElemX * elem = new HeapElemX(index, phi[index]);
							elem->prev_ind = prev_ind;
							heap.insert(elem);
							elems[index] = elem;
