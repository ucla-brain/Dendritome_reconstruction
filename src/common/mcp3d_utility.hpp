...
public:
    ...
    // terminal directory:
    // (1) contain only files or empty directories
    // (2) has subdirectories but all of them already expanded
    ...
    std::string& TerminalFiles()  { return entries_in_terminal_dir_; }
...
private:
    ...
    std::string entries_in_terminal_dir_;
};
#endif

std::string NormPathSeparatorOS(const std::string& input_path);

std::string NormPathDriveOS(const std::string& input_path);

/// (1) replace network drive
///     windows: /ifs/loni/faculty/dong/mcp -> Z:backslash
///     linux: Z:\ -> /ifs/loni/faculty/dong/mcp
/// (2) replace path separator
std::string NormPathOS(const std::string& input_path);

boost::filesystem::path NormBoostPathOS(const std::string& input_path);

inline std::string CurrentWorkingDir()
{ return boost::filesystem::current_path().string(); }

/// boost::filesystem::is_regular_file entirely resolves symlink in file_path
/// for a symlink eventually pointer to a file,
/// both boost::filesystem::is_regular_file and boost::filesystem::is_symlink
/// return true
bool IsFile(const std::string &file_path);

/// boost::filesystem::is_directory entirely resolves symlink in file_path
/// for a symlink eventually pointer to a directory,
/// both boost::filesystem::is_directory and boost::filesystem::is_symlink
/// return true
bool IsDir(const std::string &dir_path);

/// return true if dir_path is a directory and is empty
bool EmptyDir(const std::string &dir_path);

boost::filesystem::path ResolveSymlink(const std::string &path_);

/// if input_path is not a symlink, return empty string. otherwise return
/// from boost::filesystem::read_symlink. this operation reads the immediate
/// target pointed by the link and doesn't follow to link chain end
boost::filesystem::path ReadSymlink(const std::string& input_path);
...
/// foo/ -> foo. on windows, [A-Z]:\ is considered drive root, while if '\'
/// is at the beginning of the string it will be removed
/// // -> /
/// ./foo/bar -> .
/// C:\something -> C:backslash
/// \something\else -> something
std::string LeadingDir(const std::string &file_path);
...
/// if directory already exist, return true directly
/// (vs boost::filesystem::create_directories returns false on existing input)
/// boost::filesystem::create_directories(directory): from its documentation
/// (Establishes the postcondition by attempting to create the directory p
/// resolves to, as if by ISO/IEC 9945 mkdir()). mkdir() resolves symbolic
/// link contained in p.
/// multi-thread/processor calls return true if one of the threads succeeds
/// errors thrown by boost::filesystem::create_directories are caught and result
/// in false return value. caller can assert the return value on its own
bool MakeDirectories(const std::string &dir_path);

///  path_ can be directory or file. if path_ does not exist, return true directly
/// (vs boost::filesystem::remove/remove_all returns false on non exist input)
/// if remove_link_target is true and path_ is a symlink, the symlink is first
/// removed, and its target sent to RemovePath again recursively, resulting
/// in the deletion of the symlink chain and final target.
/// otherwise only the symlink or the non symlink path_ is removed.
/// return true if all underlying boost::filesystem and recursive calls return true
/// multi-thread/processor calls return true if one of the threads succeeds
/// errors thrown by boost::filesystem::remove/remove_all are caught and result
/// in false return value. caller can assert the return value on its own
bool RemovePath(const std::string &path_, bool remove_link_target = true);

void AsyncRemovePath(const std::string &path_, const std::string &mode = std::string());
...
MCP3D_EXPORT
std::vector<std::string> FilesInDir(const std::string &directory, bool full_path = true, bool sort_files = true,
                                    const std::vector<std::string> &incl = std::vector<std::string>(), int n = -1,
                                    const std::vector<std::string> &excl = std::vector<std::string>());

MCP3D_EXPORT
std::vector<std::string> FilesEndsWithInDir(const std::string &directory, const std::string& end_str,
                                            bool full_path = true, bool sort_files = true, int n = -1);

MCP3D_EXPORT
std::vector<std::string> DirsInDir(const std::string &directory_, bool full_path = true, bool sort_files = true,
                                   const std::vector<std::string> &incl = std::vector<std::string>(), int n = -1,
                                   const std::vector<std::string> &excl = std::vector<std::string>());
...
/// e.g. /foo/bar, /foo -> bar
std::string DiscardLeadingComponent(const std::string &path_, const std::string &component_);
...
template<typename T>
std::string JoinVector(const std::vector<T> &v, const std::string &d = std::string(), bool brackets = false);

template <>
std::string JoinVector<std::string>(const std::vector<std::string>& v, const std::string &d, bool brackets);

template<typename T>
std::string JoinArray(const std::unique_ptr<T[]> &v, int n, const std::string &d = std::string(), bool brackets = true);
...
template<typename T1, typename T2, typename T3>
std::vector<T1> SubtractSeq(const std::vector<T2> &lhs, const std::vector<T3> &rhs);
...
template<typename T1, typename T2, typename T3>
std::vector<T1> MultiplySeq(const std::vector<T2> &lhs, const std::vector<T3> &rhs);
...
template<typename T1, typename T2, typename T3>
std::vector<T1> DivideSeq(const std::vector<T2> &numerator, const std::vector<T3> &denominator);
...
/// be returned as "012"
std::string PadNumStr(int64_t n, int width);
...
void RenameFilesToSortable(std::vector<std::string> &file_list, const std::string &prenumber = std::string(),
                           const std::string &postnumber = std::string(), bool sort_files = true);
...
/// " ab c " -> "ab", "c"
/// ",ab,c," -> "", "ab", "c"
std::vector<std::string> SplitString(const std::string &in_string, const std::string &delim = std::string());
...
/// remove leading white spaces (' ' and '\n')
...
/// no revert option
std::string ListDirString(const std::string &dir_path, bool sort_alpha = false, bool list_directories_first = true);
...
template <typename T>
std::string mcp3d::JoinVector(const std::vector<T>& v, const std::string& d, bool brackets)
{
...
template<typename T>
std::string mcp3d::JoinArray(const std::unique_ptr<T[]> &v, int n, const std::string &d, bool brackets)
{
    std::string delim(d), output = brackets ? "[" : "";
    if (delim.empty())
        delim = " ";
...
template<typename T1, typename T2, typename T3>
std::vector<T1> mcp3d::AddSeq(const std::vector<T2> &lhs, const std::vector<T3> &rhs)
{
...
template<typename T1, typename T2, typename T3>
std::vector<T1> mcp3d::SubtractSeq(const std::vector<T2> &lhs, const std::vector<T3> &rhs)
{
    static_assert(std::is_arithmetic<T1>() && std::is_arithmetic<T2>() &&
                  std::is_arithmetic<T3>(),
                  "container elements must be arithmetic type");
...
template<typename T1, typename T2, typename T3>
std::vector<T1> mcp3d::MultiplySeq(const std::vector<T2> &lhs, const std::vector<T3> &rhs)
{
    static_assert(std::is_arithmetic<T1>() && std::is_arithmetic<T2>() &&
                  std::is_arithmetic<T3>(),
                  "container elements must be arithmetic type");
...
template<typename T1, typename T2, typename T3>
std::vector<T1> mcp3d::DivideSeq(const std::vector<T2> &numerator, const std::vector<T3> &denominator)
{
    static_assert(std::is_arithmetic<T1>() && std::is_arithmetic<T2>() &&
...
void AsyncRemovePathMPI(const std::string &path_, MPI_Comm comm = MPI_COMM_WORLD, const std::string &mode = std::string());
